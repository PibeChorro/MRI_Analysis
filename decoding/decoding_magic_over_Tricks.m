...
for s = 1:length(subNames)
    ...
    cfg.design.train        = double(horzcat(train_1,   train_2));
    cfg.design.test         = double(horzcat(test_2,     test_1));
    cfg.design.label        = double(repmat (cfg.files.label,1,2));

    %% This creates the leave-two-runs-out cross validation design:
    %cfg.design = make_design_cv_2chunksOut(cfg);
